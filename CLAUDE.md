...
Session start: before your first response, also read the "Working With Tom & How This Project Operates" section at the top of REFERENCE.md. It covers how to collaborate, the document hierarchy (CLAUDE.md → REFERENCE.md → docs/LAUNCH_READINESS.md), how to approach problems, and what NFL Season 2 is validating. docs/LAUNCH_READINESS.md is the source of truth for current state — never trust a frozen snapshot
...
22. **Season phases are sequential and admin-initiated** — `OFF_SEASON → PRE_SEASON → REGULAR → REGULAR_COMPLETE → PLAYOFFS → SUPERBOWL_INTRO → SUPERBOWL → SEASON_COMPLETE`. OFF_SEASON is the quiet calendar window before exhibition games. PRE_SEASON is an idle bridge phase — **no picks are possible in it** (`admin_advance_season_phase` forces `week_state='idle'`, `picks_open=false`). Preseason GAMEPLAY is a separate competition (`nfl_2026_pre`) running in REGULAR phase with weeks 1–3 and `espn_season_type='1'`. Isolation from the live season comes from the competition string, never from `current_phase`. Verified 2026-07-16. Weekly cycle only runs inside REGULAR, PLAYOFFS, and SUPERBOWL.
    - **Corollary — never ask `current_phase` whether a week is running; ask `week_state`.** A preseason competition is permanently `REGULAR` by design, so the phase *cannot* express "no week is running" and any gate built on it silently fails there while testing fine on `nfl_2026`. Three separate bugs have come from exactly this: the period labels (pill read `W01`), the Picks holding screen (rendered a live game list against an idle week), and the Ladder pre-launch copy (mid-season "scores will appear" before a single pick existed). `week_state === 'idle'` is the safe test — a live week cycles `picks_open → locked → live → settling → complete → picks_open` and never returns to `idle`, so it cannot false-positive mid-season.
...
24. **Partners are managed by Directors — there is no Chairman.** A partner's board lives in `partner_members` (role `'director'`, one or more), independent of any Club Pool — sponsor-only partners have Directors too. HotPick staff seed the first Director by email in `PartnerAdminScreen` (`pending_role_grants` claims at signup); any Director adds or removes Directors via League Tools, and the last Director cannot be removed (server-enforced). Directors manage perk + broadcasts via League Tools (`ClubAdminScreen`); `_caller_can_manage_partner` and `send-partner-broadcast` gate on `partner_members` membership. Partners attract pools to their roster, not the other way around — `pools.partner_id` is set by the pool's organizer via `PartnerDirectoryScreen`, never by super-admin push. Each partner has at most one Club Contest (`partners.club_pool_id`); its Gaffer defaults to a live Director at creation and is reassignable (`setClubPoolGaffer`). Participation is optional at every seat: Director does not imply Contest membership; Gaffer does not imply picking. (Pool-level delegation — Gaffer / Assistant Gaffer via `pool_members` — is unchanged.) [Amended 2026-08-22, Tom-authorized; replaces the Chairman model.]
...
- **Deep links never reach JavaScript** — `expo-linking` is not installed, and `AppDelegate.swift` does not override `open url` or `continueUserActivity`, so nothing forwards URLs from `ExpoAppDelegate` into React Native's `Linking`. iOS opens the app correctly for both `hotpick://` and `https://hotpick.app`, then the URL is dropped and `handleDeepLink` never fires. This breaks tap-to-join invite links and password reset; **manual invite-code entry is unaffected**. Verified on device 2026-08-05 across custom scheme, universal link, and email link. The fix is native and therefore **[RESUBMIT]** — see DEEPLINK-01. Until it ships, share invite codes as text, never as links. Note that an app opening from a link proves only that iOS matched the URL; it says nothing about whether JavaScript received it.

...
- **Deep links reach JavaScript on cold launch only** — iOS delivers a launch URL via `launchOptions`, which `AppDelegate.swift` already forwards through `startReactNative`, so tapping a link with the app fully closed works end to end. When the app is already running or backgrounded, iOS instead calls `application(_:open:options:)` / `application(_:continue:restorationHandler:)`, neither of which `AppDelegate.swift` overrides, so the URL is dropped silently — no error, no navigation, nothing. Affects tap-to-join invite links and password reset **in the warm case only**. Verified on device: 2026-08-05 (warm — five probes, three link types, zero fires) and 2026-08-06 (cold — full chain to Set-new-password; password changed and signed in with). Manual invite-code entry is unaffected. Android is **unverified, not working-or-broken**: the 2026-08-06 Android test ran on a 29 July bundle predating the `token_hash` branch, so its result is uninterpretable. The fix is native and therefore **[RESUBMIT]** — see DEEPLINK-01 v2. Until it ships, share invite codes as text, not links, and keep `SHOW_FORGOT_PASSWORD = false`.
  - Two testing lessons, worth keeping. An app opening from a link proves only that iOS matched the URL — it says nothing about whether JavaScript received it. And a link working once proves only that it worked in that launch state; **warm and cold are separate paths and must be tested separately**.

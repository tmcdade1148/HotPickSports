...
- **Deep links do not reach JavaScript** — `expo-linking` is not installed (absent from both `package.json` and `Podfile.lock`), so nothing forwards URLs from `ExpoAppDelegate` to React Native's `Linking`. iOS opens the app correctly for both `hotpick://` and `https://hotpick.app` links, then the URL is dropped before JS sees it, and `handleDeepLink` in `RootNavigator` never fires. This breaks password reset and tap-to-join invite links; **manual code entry is unaffected**. Verified on device 2026-08-05 — Alert probes on every branch of that path, none fired, for either link type. Fixing it is a native dependency change and therefore **[RESUBMIT]**, not OTA. Until then, share invite codes as text, not as links. Note the diagnostic trap: every file in the deep-link path reads correctly, because none of it is ever reached — do not debug that code, check whether the handler fires at all first.

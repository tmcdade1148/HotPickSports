...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // finalize_latest_completed_week returns "Finalized ... week N" on success,
    // or "No completed unfinalized week found" / "Skipped: ..." otherwise.
    // Without this gate a frequent cron posts "Week N is official" to every
    // pool on every run.
    const didFinalize = typeof data === "string" && data.startsWith("Finalized");

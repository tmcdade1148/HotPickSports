-- finalizer_hotpick_correct_null
--
-- Finding T1-2 (260611 Pre-Launch Fix Batch, Fix C): finalize_week_for_all_users
-- wrote is_hotpick_correct = false for users who had NO HotPick that week,
-- conflating two distinct states. The live incremental scorer (_shared/scoring.ts
-- via upsert_season_week_scores) correctly leaves null. This aligns the finalizer
-- to that semantics:
--   null  = no HotPick that week (no-show, or picks without a HotPick) OR in progress
--   true  = had a HotPick and it won  (after FINAL)
--   false = had a HotPick and it lost (after FINAL)   ← legitimate false, preserved
--
-- This never violated "never false before FINAL" — the finalizer runs only after
-- the v_final_games = v_total_games safety check — so it is a semantic/display
-- correctness fix, not a scoring-before-FINAL bug. Downstream is already
-- null-safe and benefits: compute-hardware counts a HotPick attempt only when
-- is_hotpick_correct IS NOT NULL (so no-show weeks no longer inflate the HotPick
-- denominator), and get_week_leaderboard maps the flag through an ELSE NULL CASE
-- with no false-filter.
--
-- Three surgical changes vs. the live function (everything else byte-identical):
--   1. no-show INSERT: is_hotpick_correct false -> NULL
--   2. week_summary: BOOL_OR(is_hotpick AND points>0) -> NULL when the user made
--      no HotPick (CASE WHEN BOOL_OR(is_hotpick) ... ELSE NULL)
--   3. has-picks INSERT: drop COALESCE(..., false) so a NULL stays NULL
-- The live scorer is NOT touched (Fix C red flag). Signature, attributes
-- (no SECURITY DEFINER, SET search_path = public) and grants are preserved by
-- CREATE OR REPLACE.
